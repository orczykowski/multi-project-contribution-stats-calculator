...
public record UserContributionStats(User user,
  ...
  public UserContributionStats(final User user, final ContributionStats counts, final ProjectDistributionStats distribution) {
    Preconditions.checkRequiredArgument(user);
    this.user = user;
    this.counts = Objects.requireNonNullElseGet(counts, ContributionStats::empty);
    this.distribution = Objects.requireNonNullElseGet(distribution, ProjectDistributionStats::empty);
  }
  ...
  public UserContributionStats add(final UserContributionStats stats) {
    ...
    return new UserContributionStats(this.user, this.counts.add(stats.counts), ProjectDistributionStats.empty());
  }
