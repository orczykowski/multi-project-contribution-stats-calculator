...
import static io.github.orczykowski.adapter.writer.PdfReportLabel.*;
import static io.github.orczykowski.core.project.ReportFormat.PDF;
  ...
  private Element createProjectContribution(final ContributionReport report) {
    ...
    private Stream<Object> userProjectDetailsStats(final int idx,
        ...
        return Stream.of(idx + 1, userStat.user().name(),
                counts.commits(), counts.linesAdded(), counts.linesRemoved(),
                counts.filesChanged(), counts.newFiles(),
                counts.productionLinesAdded(), counts.testLinesAdded(), counts.configLinesAdded(),
                distribution.commitsParticipation(), distribution.linesAddedParticipation(),
                distribution.linesRemovedParticipation());
    }

    private Stream<Object> contributionStatsAsStreamValues(final ContributionStats stats) {
        return Stream.of(stats.commits(), stats.linesAdded(), stats.linesRemoved(),
                stats.filesChanged(), stats.newFiles(),
                stats.productionLinesAdded(), stats.testLinesAdded(), stats.configLinesAdded());
    }
