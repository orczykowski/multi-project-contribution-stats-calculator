...
import static pl.boringstuff.utils.UserStatsAssertion.assertUserContributionStats
...
"""
  @Subject
  RawCsvStatsResultParser parser = RawCsvStatsResultParser.getInstance()

  def "should parse csv project stats to ProjectStats"() {
    when:
      def result = parser.parse(csv)
  ...
  def "should return empty list for empty csv"() {
    when:
      def result = parser.parse(emptyCsv)

    then:
      result.isEmpty()

    where:
      emptyCsv << ["", " ", null, "name,loc,commits,files,distribution"]
  }
