...
class UserContributionStatsSpec extends Specification {
    ...
    def "should create user stats with all possible stats"() {
        given:
        def contributionStats = createContributionStat(767, 101, 3)
        def dictStats = createDistributionStat(15.1, 11.0, 1.0)
    ...
    def "should add count user stats when adding two user stats"() {
        given:
        def someContributionStats = createContributionStat(43, 55, 14)
        ...
        UserStatsAssertion.assertUserContributionStats(result)
                .hasUser(USER_HULK)
                .hasContributionStats(new ContributionStats(44L, 56L, 15L, 0L, 0L, 0L, 0L, 0L))
    }

    def "should set as empty distribution stats when add two user stats"() {
        given:
        def someContributionStats = createContributionStat(0, 10, 0)
    ...
    private static ContributionStats createContributionStat(Long commits = 1, Long linesAdded = 0, Long linesRemoved = 0) {
        new ContributionStats(commits, linesAdded, linesRemoved)
    }

    private static ProjectDistributionStats createDistributionStat(BigDecimal commits = 1, BigDecimal linesAdded = 0.0,
                                                                    BigDecimal linesRemoved = 0.0) {
        new ProjectDistributionStats(commits, linesAdded, linesRemoved)
    }
}
